			...
			$et_content .='<div class="slide-homepage" style="display : none;">
				...
				$et_content .='</ol>
					...
					foreach($posts as $post){					
						$et_link	= get_post_meta($post->ID,'et_link',true);	
						...
						$class = ($j==0) ? 'active' : ''; 
					 	...
					 	$et_des = break_text($post->post_content,225);
					 	if(phpversion('tidy') =='2.0')
							{								
								
								$tidy = new tidy();
								$tidy->parseString($et_des,array('show-body-only'=>true),'utf8');
								$tidy->cleanRepair();	
								$et_des 	=	$tidy->value;
							}
					    ...
					    if($hide_text == false) {
							$et_content .= '<div class="carousel-caption">											
								...
								<div class="btn-read-more"><a href="'.esc_url($et_link).'">READ MORE<span class="icon" data-icon="]"></span></a></div>
							</div>';
						 }
