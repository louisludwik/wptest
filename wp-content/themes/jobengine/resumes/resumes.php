                ...
                ?>
                    ...
                    <div class="timeline-item exp-item">
                        ...
                        <div>
                            ...
                            <label for="experience"><?php _e('Responsabilities', ET_DOMAIN) ?></label><br />

                            <textarea class="resume-info" name="experience[responsabilities][]"><?php echo !empty($exp['responsabilities']) ? $exp['responsabilities'] : '' ?></textarea>

                        </div>
                        
                        <div>

                            <label for="experience"><?php _e('Achievements', ET_DOMAIN) ?></label><br />

                            <textarea class="resume-info" name="experience[achievements][]"><?php echo !empty($exp['achievements']) ? $exp['achievements'] : '' ?></textarea>
                        ...
                        <div class="year">
                            Years Experience: 
                            <?php 
                                if ( !empty($exp['from']['year']) ) {
                                    if ( !empty($exp['to']['year']) ) {
                                        echo $exp['to']['year'] - $exp['from']['year'];
                                    } else {
                                        echo (int)date('Y') - $exp['from']['year'];
                                    }
                                }
                            ?>
                        </div>
        ...
        <script type="text/template" id="exp_template">
            ...
            <div class="timeline-item exp-item">
				...
				<div class="timeline-date">
                    ...
                    <textarea name="experience[responsabilities][]" id=""></textarea>
				...
				<div class="timeline-date">
					...
					<textarea name="experience[achievements][]" id=""></textarea>
        ...
        foreach ($_POST['experience']['name'] as $index => $value) {
            ...
            if (!empty($value) && (!empty($_POST['experience']['from']))) {
                ...
                $exp[] = array(
					...
					'responsabilities'  => $_POST['experience']['responsabilities'][$index],
					
					'achievements'  => $_POST['experience']['achievements'][$index],
...
class JE_Resume_Timeline{
    ...
    protected function _output($fields){
        ...
        if ( $return['from']['month'] == '' )
            ...
            $return['from']['display'] = date('M', mktime(0, 0, 0, $return['from']['month'], 1)) . '-' . $return['from']['year'];
        ...
        if ( $return['to']['month'] == '' )
            ...
            $return['to']['display'] = date('M', mktime(0, 0, 0, $return['to']['month'], 1)) . '-' . $return['to']['year'];
...
class JE_Resume_Experience extends JE_Resume_Timeline{
    ...
    public function __construct(){
        ...
        parent::__construct('experience', apply_filters( 'et_experience_fields', array('name', 'from', 'to', 'current', 'position', 'responsabilities','achievements') ));
        ...
        if($resume) {
            ...
            if( !empty($resume->et_experience) ) {
                foreach ((array)$resume->et_experience as $exp) { 
                    ...
                    if ( $exp['current'] ) {
                        $total_years += (int)date('Y') - $exp['from']['year'];
                        $total_month += (int)date('m') - $exp['from']['month'];
                    } else {
                        $total_years += $exp['to']['year'] - $exp['from']['year'];
                        $total_month += $exp['to']['month'] - $exp['from']['month'];
                    }
                    if ( $total_month > 12 ) {
                        $total_years ++;
                        $total_month -= 12;
            ...
            $str_exp = $total_years > 0 ? $total_years . " year(s) " : "";
            $str_exp .= $total_month > 0 ? $total_month . " month(s) " : "";
            $response   =   array(

                'success'   => true,

                'msg'       => __("Your resume has been updated.", ET_DOMAIN),
                'data'      => array('strexp' => $str_exp, 'resume' => $resume ,  'jobseeker' => JE_Job_Seeker::convert_from_user(get_userdata($resume->post_author)) )
            );
