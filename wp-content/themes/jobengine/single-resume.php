...
if (have_posts()) { 
	...
	// get the resume
		...
		//anonymous - hide name
		/*if ($resume->anonymous[0]->name == 'Active') {
	...
	<div  class="main-center clearfix padding-top30 jse-profile <?php if ($authorise) echo 'authorized' ?>">
		...
		<div id="resume_profile" class="jse-account main-column jse-profile-main jse-signup-profile">
			...
			*/
			...
			<div class="module module-edit education">
				<div class="title">
					<?php _e('Work Experience', ET_DOMAIN); ?> <span id="total_exp"><?php if (!empty($str_exp)) echo $str_exp ;?></span>
				...
				<div class="edu-module cnt">
					...
					foreach ((array)$resume->et_experience as $exp) { 
						...
						//if ( ( $resume->anonymous[0]->name == 'Active' ) && ($exp['current'] == 1) ) continue;
			//The entire work experience is hidden if the job seeker selects the CONFIDENTIAL MODE in his profile page
						...
						<div class="item">
							...
							<div class="content">
								<div class="year">
                                    ...
                                    $months = 0;
                                    if ( $exp['current'] == 1 ) {
                                        $years = (int)date('Y') - $exp['from']['year'];
                                        $months = (int)date('m') - $exp['from']['month'] + 1;
                                        if ( $months < 0 ) {
                                            $years--;
                                            $months += 12;
                                        }
                                    } else {
                                        $years = $exp['to']['year'] - $exp['from']['year'];
                                        $months = $exp['to']['month'] - $exp['from']['month'] + 1;
                                        if ( $months < 0 ) {
                                            $years--;
                                            $months += 12;
                                        }
		...
		<div class="content">
			<div class="year">
                <# 
                    ...
                    var m = 0;
                    if (current == 1) {
                        y = new Date().getFullYear() - from.year;
                        m = new Date().getMonth()+1 - from.month + 1;
                    } else {
                        y = to.year - from.year;
                        m = to.month - from.month + 1;
                    }
                    if ( m < 0 ) {
                        y--;
                        m += 12;
                    }
                    var str = y==0 ? "" : y + " year(s) ";
                    str += m==0 ? "" : m + " month(s)";
                #>
				{{ from.display }} - {{ to.display }} {{ str }}
			</div>
			<div class="school">{{ highlight }}</div>
		</div>
	</div>
