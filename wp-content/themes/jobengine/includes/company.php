...
function et_ajax_user_register() {
	...
	try {
		...
		*/
        ...
        $user     = et_login($_REQUEST['user_name'], $_REQUEST['user_pass'], true);
        if( is_wp_error($user) ) {
            // apply login by email here
            $user     = et_login_by_email($_REQUEST['user_email'], $_REQUEST['user_pass'], true);
        }
	...
	if ( empty( $user->ID ) ){
		...
		if( $flush || !$company_response ) {
			...
			$company_response	=	array(
				...
				'profile_url'		=> et_get_page_link('profile'),
