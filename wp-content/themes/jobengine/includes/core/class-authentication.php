...
function et_register( $userdata, $role = 'subscriber', $auto_login = false ){
	extract($userdata);	/*if (!preg_match("/^[a-zA-Z0-9_]+$/", $userdata['user_login'])){		return new WP_Error('username_invalid', __('Username is invalid', ET_DOMAIN));	}*/
    $userdata['user_login'] = $userdata['user_email'];	$userdata['role']	= $role;
...
function et_retrieve_password() {
	...
	if ( empty( $_POST['user_login'] ) ) {		$errors->add('empty_username', __('<strong>ERROR</strong>: Enter email address.', ET_DOMAIN));	} else if ( strpos( $_POST['user_login'], '@' ) ) {
	...
	if ( !$user_data ) {		$errors->add('invalidcombo', __('<strong>ERROR</strong>: Invalid email address.', ET_DOMAIN));		return $errors;
	}
