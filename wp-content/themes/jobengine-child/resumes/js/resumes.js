...
// when having the id, fetch into this object as a company model
	...
	parse       : function(res){
		...
		if(!res.success){
            ...
            jQuery('#total_exp').html(res.data.strexp);
			pubsub.trigger('je:notification', {
...
JobEngine.Views.ExperienceView = Backbone.View.extend({
	...
	toObject: function(){
		...
		this.data.responsabilities 	= element.find('textarea.responsabilities').val();
		
		this.data.achievements 	= element.find('textarea.achievements').val();
        ...
        if ( this.data.current ) {
            if ( parseInt(this.data.fromYear) > 0 ) {
                y = new Date().getFullYear() - this.data.fromYear;
                m = new Date().getMonth()+1 - this.data.fromMonth + 1;
            } else {
                this.data.yearexp = '';
            }
        } else {
            if ( parseInt(this.data.fromYear) > 0 && parseInt(this.data.toYear) > 0 ) {
                y = this.data.toYear - this.data.fromYear;
                m = this.data.toMonth - this.data.fromMonth + 1;
            } else {
                this.data.yearexp = '';
            }
        }
        ...
        this.data.yearexp = y == 0 ? "" : y + " years ";
        this.data.yearexp += m == 0 ? "" : m + " months";
        element.find('span.year_exp').html(this.data.yearexp);
